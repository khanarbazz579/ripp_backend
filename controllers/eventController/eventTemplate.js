...
const frontendUrl = "https://frontend.devsubdomain.com/response-event-invitation/";
//const frontendUrl = "http://localhost:4200/response-event-invitation/";

emailTemplate.inviteTemplate = async function(timeTitle, start, end, title, users, text, address, key) {
    var d = moment.utc(start);
   ...
   <table style="width: 100%;max-width: 1000px;background-color: #f3f7f9;margin: 0 auto;" cellspacing="0" cellpadding="0" border="0">
      <tbody>
         ...
         <tr>
            <td width="20px"></td>
            <td height="25px">
            <a href="${frontendUrl}${key}" style="background: #2563e4;font-size: 14px;color: #fff;padding: 7px; text-decoration:none;">Click Here To Respond</a>
            </td>
            <td width="20px"></td>
         </tr>
         ${textArea}
...
emailTemplate.orgenizerTemplate = async function(timeTitle, start, end, title, users, text, address, userName, is, rmessage) {
    var d = moment(start);
    var e = moment(end);
    var monthName = d.format('MMMM');
    var dayName = d.format('dddd');
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        if (users[i].fixed) {
            ...
            if (users[i].status == 'U') {
                ...
                isStatus = 'Not Attending <br>';
                messageDetails = `<b>Reason: </b>  ${rmessage}`;
            } else if (users[i].status == "T") {
                isStatus = 'Possibly Attending';
            } else {
                isStatus = 'Attending';
            }
            who += `<li style="font-size: 16px;line-height: 40px;">${users[i].first_name} ${users[i].last_name != null ? users[i].last_name : ''} - ${users[i].company != null ? users[i].company : ''} - ${users[i].email} <span style="font-weight: 600; color: rgb(177, 177, 177);"> </span> <b>${isStatus}</b> ${messageDetails} </li>`;
        }
    ...
    let agenda = '';
    if (text) {
        agenda = ` <div style="width: 100%;min-height: 150px;background: #fff;border: 1px solid #ddd; white-space: pre-wrap;">${text}</div>`;
    }
