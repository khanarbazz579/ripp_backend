...
const preparingZip = async (req, res) => {
    ...
    if (requestedFiles.length > 0) {
        ...
        s3Zip
            .archive({ s3: s3Client, bucket: 'devsubdomainfiles' }, '', filesAwsPath)
            .pipe(output);

        output.on('finish', function () {
            console.log('archiver has been finalized and the output file descriptor has closed.');
            if (process.env.NODE_ENV !== 'test') {
                io.emit('zippedFileCreated', zippedFileName);
            }
        });
...
// const unlinkZippedFile = (req, res) => {
//     var filename = req.params.filename + '.zip';
//     var file = 'public/zipped/' + filename;
//     fs.unlinkSync(file);
...
async function getFilesPathInArray(requestedFiles, userid) {
    ...
    let [error, filesData] = await commonFunction.mediaCommonFunction.getFileFolderDataByQuery(query);
    if (error) {
    ...
    for (var i = 0; i < filesData.length; i++) {
        ...
        filesAwsPath.push(filesData[i].dataValues.file_property.dataValues.path);
    }
    return filesAwsPath;
}
