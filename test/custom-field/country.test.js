...
describe('Countries', async () => {
	...
	describe('/LOGIN Countries', () => {

		before((done) => {
			commonFunction.sequalizedDb(['histories', 'call_outcomes_transitions', 'tasks', 'users', 'user_roles']).then(() => {
	...
	describe('/GET ALL Countries', () => {
		it('it should not GET ALL Countries without access token', () => {
			return chai.request(server)
				.get('/api/countries')
				.end((res) => {
