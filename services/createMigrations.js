...
var sequelize = new _sequelize2.default(config.name, config.username, config.password, config);

const path = require('path');

const directory = './migrations';
// delete all file in migrations folder
_fs.readdir(directory, (err, files) => {
  if (err) throw err;

  for (const file of files) {
    _fs.unlink(path.join(directory, file), err => {
      if (err) throw err;
    });
  }
});
...
var _loop = function _loop(model) {
  ...
  _fs2.default.writeFileSync("./migrations/" + filename + ".js", template);
};
